...
const char * const STANCECOIN_CONF_FILENAME = "stancecoin.conf";
const char * const STANCECOIN_PID_FILENAME = "stancecoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "stancecoind -foo=bar
...
#else
    const char* pszModule = "stancecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Stancecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Stancecoin
    // Mac: ~/Library/Application Support/Stancecoin
    // Unix: ~/.stancecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Stancecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Stancecoin";
#else
    // Unix
    return pathRet / ".stancecoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", STANCECOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", STANCECOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Stancecoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Stancecoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Stancecoin Core developers";
    }
