#!/usr/bin/env bash
# Copyright (c) 2016 The Stancecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   stancecoincore.org and stancecoin.org and compares them.
...
WORKINGDIR="/tmp/stancecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://stancecoincore.org"
HOST2="https://stancecoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="stancecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'stancecoin-',
...
if [ $? -ne 0 ]; then
   echo "stancecoin.org failed to provide signature file, but stancecoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "stancecoin.org and stancecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Stancecoin Core binary release signing key installed?"
   fi
